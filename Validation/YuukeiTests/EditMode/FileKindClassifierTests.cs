...
namespace Yuukei.Tests.EditMode
{
    public sealed class FileKindClassifierTests
    {
        ...
        public void Classify_UsesSpecBuckets()
        {
            ...
            try
            {
                Assert.That(FileKindClassifier.Classify(tempDirectory), Is.EqualTo("folder"));
                Assert.That(FileKindClassifier.Classify(imagePath), Is.EqualTo("image"));
                Assert.That(FileKindClassifier.Classify(docPath), Is.EqualTo("document"));
                Assert.That(FileKindClassifier.Classify(archivePath), Is.EqualTo("archive"));
                Assert.That(FileKindClassifier.Classify(modelPath), Is.EqualTo("model"));
                Assert.That(FileKindClassifier.Classify(Path.Combine(tempDirectory, "unknown.bin")), Is.EqualTo("other"));
            }
            finally
            {
