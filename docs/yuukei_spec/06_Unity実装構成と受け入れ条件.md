...
- 特定アプリ使用中検知
- ファイル D&D 種別判定
- `character_clicked` への `_event_body_part` 注入
- drag / choice overlay / settings と競合しない gesture 抑制
...
3. クリック / なでなで / ダブルクリック / ドラッグ開始 / ドラッグ終了 / ファイル D&D / 放置 / 定期発火が Daihon に届く  
...
10. DLL 警告導線が存在する  
11. `character_clicked` の context から `_event_body_part` を取得できる  
12. 頭上の左右なで gesture が `character_stroked` として自然な頻度で発火する  
11. 設定画面骨格が存在する  
